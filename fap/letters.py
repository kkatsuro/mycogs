...
'Z⅄XMΛ∩⊥SᴚΌԀONW˥⋊ſIH⅁ℲƎᗡƆᙠ∀zʎxʍʌnʇsɹbdouɯlʞɾıɥɓɟǝpɔqɐ ',  # special function for this one?

fontdict = dict()
for i, letter in enumerate('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ '):
    fontdict[letter] = [ font[i] for font in fonts ]

def get_letter(letter):
    letter_list = fontdict.get(letter)
    if letter_list == None:
        return [letter] * len(fonts)
    return letter_list

# TODO:
# * add numbers (and maybe spaces?)
# * don't replace emojis
def mapfont(text):
    zipped = zip( *[ get_letter(letter) for letter in text ])
    return [ ''.join(x) for x in list(zipped) ]
